...
def seq_align(s1, s2, enable_graphics=True):
    ...
    for i in range(1, m+1):
        scores[(i, 0)] = SPACE_PENALTY * i
    for i in range(1, n+1):
        scores[(0, i)] = SPACE_PENALTY * i

    if enable_graphics:
        render_board(screen, font, s1, s2, scores)
        pygame.display.flip()
        time.sleep(1)
    ...
    for i in range(1, m+1):
        for j in range(1, n+1):
            match = scores[(i-1, j-1)] + s(s1[i-1], s2[j-1])
            delete = scores[(i-1, j)] + SPACE_PENALTY
            insert = scores[(i, j-1)] + SPACE_PENALTY
    ...
    while i>0 or j>0:
        if enable_graphics:
            tile = create_tile(font, str(scores[(i, j)]), highlight_color)
            screen.blit(tile, ((i+1)*TILE_SIZE, (j+1)*TILE_SIZE))
            pygame.display.flip()
            time.sleep(1)
        if i > 0 and j > 0 and scores[(i,j)] == scores[(i-1, j-1)] + s(s1[i-1], s2[j-1]):
            t1 += s1[i-1]
            t2 += s2[j-1]
            i -= 1
            j -= 1
        elif i > 0 and scores[(i, j)] == scores[(i-1, j)] + SPACE_PENALTY:
            t1 += s1[i-1]
            t2 += SPACE_CHAR
            i -= 1
        else:
